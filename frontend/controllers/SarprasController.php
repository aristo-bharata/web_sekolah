...
class SarprasController extends Controller
{
    ...
    public function actionIndex() {
        ...
        $query->select('articles.*, articles.id as article_id, articles.description as article_desc, master_articles.*, master_articles.id as master_article_id, medias.*, medias.id as media_id, medias.description as media_desc, files.*, files.id as file_id, files.description as file_desc, employees.*, employees.id as  employees_id, employees.description as employee_desc')
                ...
                ->where(['article_categories_id' => self::SARPRAS])
                ...
                ->all();
        $countSarpras = $query->count();
        $pagination = new Pagination([
            'defaultPageSize' => 12,
            'totalCount' => $countSarpras,
        ]);
        
        $indexSarpras = $query->offset($pagination->offset)
        ...
        $countPengumuman = count($this->getCountGenericContent(self::PENGUMUMAN));
        $indexPengumuman = $this->getPopularContent(self::PENGUMUMAN, 5);
        
        $this->bodyID = 'index-sarpras';
        
        /**
         * Meta Property
         */
        
        Yii::$app->view->title = 'Sarpras - sdn2tamanbaru.sch.id';
        Yii::$app->view->registerMetaTag([
            'name' => 'description',
            'content' => 'website resmi SDN 2 Taman Baru halaman SARPRAS'
        ]);
        
        return $this->render('index', [
            'indexSarpras' => $indexSarpras,
            'about' => self::ABOUT,
